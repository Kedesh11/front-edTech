...
const mockMessages = {
  1: [
    { from: 'parent', text: 'Bonjour, pouvez-vous m\'envoyer les devoirs de mathématiques ?', time: '09:00' },
    { from: 'teacher', text: 'Oui, je vous les transmets immédiatement.', time: '09:05' },
    { from: 'parent', text: 'Merci beaucoup ! Alex les a bien reçus.', time: '09:10' },
    { from: 'teacher', text: 'Merci pour votre retour concernant les devoirs.', time: '09:12' },
  ],
  2: [
    { from: 'parent', text: 'Bonjour, la réunion est confirmée pour vendredi ?', time: '08:00' },
    { from: 'teacher', text: 'On se voit à la réunion parents-profs.', time: '08:05' },
  ],
};

export default function ParentDiscussions() {
  const { user } = useAuth();
                  ...
                  {messages.map((msg: { from: string; text: string; time: string }, i: number) => (
                    <div key={i} className={`flex ${msg.from === 'parent' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-xs px-4 py-2 rounded-lg shadow-sm ${
                        ...
                        <p className={`text-xs mt-1 ${msg.from === 'parent' ? 'text-orange-100' : 'text-gray-500'}`}>
                          {msg.time}
                        </p>
                      </div>
                    </div>
